$(document).ready(function () {
  ...
  $(document).on(
    'click',
    '.statistics_card, .statistics_card_title',
    function () {
  ...
  $(document).on(
    'click',
    '.gal_inspection_card, .gal_inspection_card_title',
    function () {
  ...
  $(document).on(
    'click',
    '.tol_inspect_card, .tol_inspect_card_title',
    function () {
  ...
  $.getJSON('/api/stats/combined_stats_json')
    .done(function (data) {
      ...
      if (Chart.getChart('lineGraphID') !== undefined)
        Chart.getChart('lineGraphID').destroy();
  ...
  $.getJSON('tol/gal_and_partners')
    .done(function (data) {
      ...
      spinner.show();
      let map_locations = data.partner_locations_lst.concat(
        data.gal_locations_lst
      );
      let map_markers = [];
      map_locations.map((x) =>
        map_markers.push({
          ...
          style: { r: x.style.r, fill: x.style.fill },
          city: x.city,
          state: x.state,
          country: x.country,
    ...
    .fail(function (error) {
      console.error('Failed to fetch map data:', error?.message || error);
      statusText.text('Failed to load map data.');
      spinner.hide();
    });
});
