...
from common.read_utils.ena_helper import SubmissionHelper
import common.schemas.utils.data_utils as d_utils
from src.apps.copo_core.copo_lookup_service import COPOLookup
from common.lookup.lookup import SRA_SUBMISSION_TEMPLATE, SRA_EXPERIMENT_TEMPLATE, SRA_RUN_TEMPLATE, \
...
class EnaReads:
    def __init__(self, submission_id=str()):
        ...
        self.webin_user = helpers.get_env('WEBIN_USER')
        self.webin_domain = helpers.get_env('WEBIN_USER').split("@")[1]
    ...
    def process_queue(self):
        ...
        for rec in records:
            ...
            if not recorded_time:
                rec['date_modified'] = helpers.get_datetime()
        ...
        queued_record['date_modified'] = helpers.get_datetime()
    ...
    def process_study_release(self, force_release=False):
        ...
        if release_status.upper() == 'PUBLIC':
            ...
            try:
                ...
                first_public = helpers.get_datetime()
        ...
        if (release_date and release_date["in_the_past"] is True) or force_release is True:
            ...
            prj = submission_record.get('accessions', dict()).get('project', [{}])
            prj[0]['status'] = 'PUBLIC'
            prj[0]['release_date'] = helpers.get_datetime()
    ...
    def finalise_submission(self, is_new=True):
        ...
        collection_handle = ghlper.get_submission_handle()
        submission_record = dict(complete=True, completed_on=helpers.get_datetime())
        collection_handle.update(
            {"_id": ObjectId(self.submission_id)},
            {'$set': submission_record})
