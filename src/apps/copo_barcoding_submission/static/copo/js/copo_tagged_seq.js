...
$(document).ready(function () {
    ...
    var wsurl = wsprotocol + window.location.host + '/ws/tagged_seq_status/' + uid
    ...
    $(document).on("click", ".new-taggedseq-spreadsheet-template", function (event) {
        url =  "/copo/copo_taggedseq/ena_taggedseq_manifest_validate/" + uid+ "?checklist_id=" + $("#checklist_id").find(":selected").val();
        dialog.realize();
    ...
    */
    ...
    function do_record_task(event) {
        ...
        if (task == "add") {
            ...
            url = "/copo/copo_seq_annotation/ena_annotation/" + uid + "/" + records[0].record_id
            handle_add_n_edit(url)
        }
        else {    
            var args_dict = {}
