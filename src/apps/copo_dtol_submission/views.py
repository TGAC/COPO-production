...
def copo_sample_accept_reject(request):
    ...
    associated_profiles_type_approval_for = [
        associated_profile_type.name
        for associated_profile_type in AssociatedProfileType.objects.filter(
            is_approval_required=True, users=get_current_user()
        )
    ]
    ...
    return render(
        request,
        'copo/copo_sample_accept_reject.html',
        {
            "sample_manager_groups": sample_manager_groups,
            "associated_profiles_type_approval_for": associated_profiles_type_approval_for,
        },
    )
...
def get_dtol_samples_for_profile(request):
    ...
    if not ViewLock().isViewLockedCreate(url=url):
        ...
        if isinstance(profile, dict):
            ...
            is_associated_project_type_checker = any(
                AssociatedProfileType.objects.filter(
                    is_approval_required=True,
                    users=current_user,
                    name__in=associated_profiles,
                )
            )
        ...
        if profile_id and profile_id != 'None':
            ...
            if profile_type:
                ...
                if ProfileType.objects.get(
                    type=profile_type
                ).is_dtol_profile:  # if it is dtol_type
                    if not is_associated_project_type_checker:
                        ...
                        samples = Sample().get_dtol_from_profile_id(
                            ...
                            search,
                            type,
                            is_associated_project_type_checker,
                            is_sequencing_centre_checker,
                        )
...
def mark_sample_rejected(request):
    ...
    return HttpResponse(
        status=status.HTTP_400_BAD_REQUEST, content="Sample IDs not provided"
    )
...
def add_sample_to_dtol_submission(request):
    ...
    associated_profiles_type_require_approval = AssociatedProfileType.objects.filter(
        is_approval_required=True, name__in=associated_profiles
    )
    associated_profiles_type_approval_for = AssociatedProfileType.objects.filter(
        is_approval_required=True, users=current_user, name__in=associated_profiles
    )
    ...
    if sample_ids and profile:
        ...
        samples = Sample().get_all_records_columns(
            filter_by=dict(_id={"$in": sample_oids}),
            projection=dict(status=1, approval=1),
        )
        ...
        for sample in samples:
            ...
            match sample["status"]:
                case "pending":
                    ...
                    for associated_profile in associated_profiles_type_require_approval:
                        if associated_profile in associated_profiles_type_approval_for:
                            update_approval[f"approval.{associated_profile.name}"] = {
                                "user_id": user.pk,
                                "date": now,
                            }
                            update_approval_for_samples.append(sample["_id"])
                        elif not sample.get("approval", {}).get(
                            associated_profile.name, ""
                        ):
                            all_approved = False
                    if all_approved:
                ...
                case _:
                    lg.error(
                        f"Sample {sample_id} has an invalid status {sample['status']} for the current profile type {type_sub} and user {current_user}"
                    )
        ...
        if update_approval_for_samples:
            Sample().update_field(
                field_values=update_approval, oids=update_approval_for_samples
            ),
        ...
        if pending_sample_ids:
            ...
            Email().notify_manifest_pending_for_associated_project_type_checker(
                data=uri + 'copo/dtol_submission/accept_reject_sample/',
...
def delete_dtol_samples(request):
    ...
    for s in ids:
        ...
        report.append(r)
    notify_frontend(
        data={"profile_id": ""}, msg=report, action="info", html_id="sample_info"
    )
