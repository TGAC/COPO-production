...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for sam in samplesinddb:
            ...
            accessions = submit_biosample(str(sour['_id']), da.Source(), "", type="source")
            print(accessions)
            ...
            if sam['ORGANISM_PART']=="WHOLE_ORGANISM":
                da.Sample().add_field("sampleSameAs", specimen_accession, sam['_id'])
            else:
                da.Sample().add_field('sampleDerivedFrom', specimen_accession, sam['_id'])
