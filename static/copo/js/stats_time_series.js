...
async function draw_line_graph() {
  ...
  bounds.append('g').append('text').attr('class', 'y-axis-label');
  ...
  function update(metric) {
    ...
    const yAxisGenerator = d3.axisLeft().scale(yScale).ticks(10, ',d');
    ...
    tooltip.select('#metric').text(metric);

    function onMouseMove(e, datum) {
      const mousePosition = d3.pointer(e);
      const hoverDate = xScale.invert(mousePosition[0]);
      const getDistanceFromHoverDate = (d) =>
        Math.abs(xAccessor(d) - hoverDate);
      ...
      tooltip.select('#num_metric').text(closestYValue);
  ...
  const metrics = ['samples', 'datafiles', 'profiles', 'users'];
