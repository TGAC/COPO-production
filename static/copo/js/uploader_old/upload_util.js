function get_upload_box_html(box_id, panel_id) {
  var html =
    ...
    '<span class="float-end clickable panel-collapsed"><i class="glyphicon glyphicon-plus"></i></span>' +
    ...
    '<form action="/rest/receive_data_file_chunked/" method="POST" id="upload_' +
    box_id +
    ...
    '<input type="hidden" name="panel_ordering" value="' +
    box_id +
    '"/>' +
...
function get_files_table(files) {
  ...
  for (file in files) {
    ...
    str +=
      ...
      file.md5 +
      '</td>' +
      '<td class="delete_cell text-center" data-file_id="' +
      file.id +
      '">' +
