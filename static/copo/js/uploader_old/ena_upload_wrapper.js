...
function get_chunk_size(event) {
  ...
  if (upload_size < chunk_threshold) {
    chunk_size = 0;
    $(event.currentTarget).parent().parent().fileupload('option', {
      maxChunkSize: 0,
    ...
    $(document).on('click', 'form[id^=upload]', function () {
      $(this)
        .fileupload({
          ...
          fail: function (e, data) {
            BootstrapDialog.show({
              ...
              message:
                'For some reason this upload failed. Please try again. If you are using a laptop, make sure you leave the lid open and adjust your settings to prevent hibernation. If this problem persists please contact the administrator.',
              buttons: [
                {
                  label: 'Close',
        ...
        .on('fileuploadchunkdone', function (e, data) {
          var file_name = data.files[0].name.substr(
            0,
            data.files[0].name.indexOf('.')
          );
          //console.log(data)
  ...
  function update_html(data, tform) {
    ...
    for (var i = 0; i < x.size(); i++) {
      ...
      html +=
        ...
        x[i].id +
        "'/> <ul class='list-inline'><li><strong>" +
        x[i].name +
        "</strong></li><li class='file_size'>" +
        x[i].size.toFixed(1) +
        ' MB</li></ul>' +
        "</div><div class='col-sm-3 col-md-3 col-lg-3'>" +
        '<span class="text-right zip-image"><img src="' +
        zipping_img +
        '" height="20px" class="pull-right"/>' +
        '<h4 style="margin-right:30px">Zipping</h4></span>' +
        '<span class="text-right hash-image"><img src="' +
        hashing_img +
        '" height="20px" class="pull-right"/>' +
        '<h4 style="margin-right:30px">Hashing</h4></span></div></div>';
      insert_node = $(tform).parents().eq(2).find('.file_status_label');
      div
        .addClass('alert alert-success file_info')
        .html(html)
        .appendTo(insert_node)
        .fadeIn();
...
function get_hash(id, tform) {
  ...
  $.ajax({
    ...
    html =
      '<h5><span class="hash_span label label-success">' +
      data.output_hash +
      '</span></h5>';
...
function make_progress_bar(file_name, size) {
  html =
    '<div id="progress_info_' +
    file_name +
    '" class="progress_info">' +
    '<input type="hidden" id="upload_id_' +
    file_name +
    '" value="" />' +
    '<span id="progress_label"></span>' +
    '<span id="total_label"> of ' +
    size +
    '</span>' +
...
function make_upload_div(file_name) {
  ...
  return $('<div/>')
    .addClass('alert alert-warning file_info')
    .attr('id', 'id_' + file_name)
    .html(
      '<strong>' +
        file_name +
        "</strong><i class='fa fa-upload pull-right' aria-hidden='true'></i>"
    );
}
