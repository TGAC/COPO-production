...
(function ($) {
  ...
  proto.toggle = function (event) {
    ...
    if (!isActive) {
      if (!menuTree) menuTree = [$parent];

      if (
        touchSupport &&
        !$parent.closest('.navbar-nav').length &&
        !menuTree[0].find(backdrop).length
      ) {
        // if mobile we use a backdrop because click events don't delegate
        $('<div class="' + backdrop.substr(1) + '"/>')
          .appendTo(menuTree[0])
          .on('click', closeOpened);
      }
  ...
  proto.keydown = function (e) {
    ...
    var $items = $parent.find(
      desc1 + ', ' + '[role="menu"]' + desc + ', [role="listbox"]' + desc
    );
  ...
  proto.change = function (e) {
    ...
    $toggle.data('placeholder') == undefined &&
    ...
    if ($items.length) {
      ...
      text =
        text.length > maxItems
          ? maxText.replace('%s', text.length)
          : text.join(', ');
    }
  ...
  function positioning($menu, $control) {
    ...
    if ($menu.hasClass('pull-middle')) {
      $menu.css(
        'margin-top',
        $menu.outerHeight() / -2 - $control.outerHeight() / 2
      );
    }
  }
  ...
  function closeOpened(event, menuTree) {
    if (opened) {
      ...
      if (parent.hasClass(openClass)) parent.removeClass(openClass);
  ...
  $.fn.dropdown.clearMenus = function (e) {
    ...
    $('.' + openClass + ' ' + toggle).each(function () {
      ...
      $parent
        .removeClass('open')
        .trigger('hidden' + eventNamespace, relatedTarget);
    });
    return this;
  };
